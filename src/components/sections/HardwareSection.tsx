  ...
  return (
    <div className="pt-32 px-6 max-w-7xl mx-auto pb-20">
      ...
      >
        ...
        <h1 className="text-6xl md:text-8xl font-black text-text-primary tracking-tighter mb-6">Hardware Engineering</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-20">
        {platforms.map((platform, idx) => (
          ...
          >
            ...
            <h3 className="text-3xl font-black text-text-primary mb-4 tracking-tight">{platform.title}</h3>
            <p className="text-text-secondary mb-8 leading-relaxed font-medium">{platform.desc}</p>
            
            <div className="space-y-6 pt-6 border-t border-border-subtle">
              <div>
                <h4 className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] mb-3">Key Specs</h4>
                <div className="flex flex-wrap gap-2">
                  {platform.specs.map(spec => (
                    <span key={spec} className="px-3 py-1 rounded-lg bg-bg-card text-[10px] font-bold text-text-secondary">
                      {spec}
                    </span>
                  ))}
                </div>
              ...
              <div>
                <h4 className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] mb-3">Common Use Cases</h4>
                <div className="flex flex-wrap gap-2">
        ...
        <div className="glass-card p-10 bg-emerald-500/[0.02]">
          ...
          <div className="space-y-6">
            <div className="flex items-start gap-4">
              <div className="p-3 bg-bg-card rounded-xl text-emerald-400 font-mono text-xs font-bold">C++</div>
              <div>
                <h4 className="text-text-primary font-bold text-sm mb-1">Arduino / ESP32</h4>
                <p className="text-xs text-text-muted">Low-level control, memory efficiency, and direct register access for maximum performance.</p>
              </div>
            </div>
            <div className="flex items-start gap-4">
              <div className="p-3 bg-bg-card rounded-xl text-emerald-400 font-mono text-xs font-bold">PY</div>
              <div>
                <h4 className="text-text-primary font-bold text-sm mb-1">MicroPython / CircuitPython</h4>
                <p className="text-xs text-text-muted">Rapid prototyping and high-level logic for IoT devices and Raspberry Pi scripts.</p>
              </div>
            </div>
          </div>
        </div>
