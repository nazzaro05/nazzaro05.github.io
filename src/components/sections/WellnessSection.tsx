  ...
  <div className="pt-32 px-6 max-w-5xl mx-auto space-y-20 pb-20">
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-5xl md:text-6xl font-black text-text-primary tracking-tighter">Tech-Wellness & IoT</h1>
      </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
      <section className="glass-card p-10 border-l-4 border-emerald-500/30">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h2 className="text-3xl font-bold text-text-primary tracking-tight">Wearables & Biometrics</h2>
        </div>
        <ul className="space-y-6 text-text-muted">
          <li className="flex gap-4">
            <Heart className="text-emerald-400 shrink-0 mt-1" size={24} />
            <span><strong className="text-text-primary block mb-1">HRV Monitoring:</strong> Tracking Heart Rate Variability as a primary indicator of nervous system balance, stress resilience, and recovery quality.</span>
          </li>
          <li className="flex gap-4">
            <Activity className="text-emerald-400 shrink-0 mt-1" size={24} />
            <span><strong className="text-text-primary block mb-1">CGM Biohacking:</strong> Continuous Glucose Monitors provide real-time data on metabolic responses to nutrition, exercise, and sleep.</span>
          </li>
          <li className="flex gap-4">
            <Moon className="text-emerald-400 shrink-0 mt-1" size={24} />
            <span><strong className="text-text-primary block mb-1">Sleep Architecture:</strong> Utilizing smart rings and under-mattress sensors to analyze deep and REM sleep cycles for cognitive optimization.</span>
          </li>
        </ul>
      </section>
      ...
      <section className="glass-card p-10 border-l-4 border-sky-500/30">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h2 className="text-3xl font-bold text-text-primary tracking-tight">Smart Environments</h2>
        </div>
        <ul className="space-y-6 text-text-muted">
          <li className="flex gap-4">
            <Sun className="text-sky-400 shrink-0 mt-1" size={24} />
            <span><strong className="text-text-primary block mb-1">Circadian Lighting:</strong> IoT-enabled lighting systems that automatically adjust color temperature to align with natural biological rhythms.</span>
          </li>
          <li className="flex gap-4">
            <Wind className="text-sky-400 shrink-0 mt-1" size={24} />
            <span><strong className="text-text-primary block mb-1">Air Quality Optimization:</strong> Real-time CO2 and PM2.5 monitoring to ensure peak cognitive focus and respiratory health in workspaces.</span>
          </li>
          <li className="flex gap-4">
            <Zap className="text-sky-400 shrink-0 mt-1" size={24} />
            <span><strong className="text-text-primary block mb-1">Neuro-Feedback:</strong> Portable EEG devices that provide real-time brainwave data to train focus, flow states, and deep meditation.</span>
          </li>
        </ul>
      </section>
    </div>
    ...
    <section className="glass-card p-10">
      <h2 className="text-3xl font-bold text-text-primary mb-10 tracking-tight">Digital Ecosystem for Performance</h2>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="p-6 bg-slate-900/40 rounded-3xl border border-border-subtle hover:border-emerald-500/20 transition-all">
          <Smartphone className="text-emerald-400 mb-4" size={32} />
          <h4 className="font-bold text-text-primary text-lg mb-3">Digital Detox</h4>
          <p className="text-sm text-text-muted leading-relaxed">Selective notification filtering and screen-time analytics to protect cognitive bandwidth.</p>
        </div>
        <div className="p-6 bg-slate-900/40 rounded-3xl border border-border-subtle hover:border-sky-500/20 transition-all">
          <Brain className="text-sky-400 mb-4" size={32} />
          <h4 className="font-bold text-text-primary text-lg mb-3">Neuro-Acoustics</h4>
          <p className="text-sm text-text-muted leading-relaxed">Binaural beats and specialized frequencies designed to induce specific neural states.</p>
        </div>
        <div className="p-6 bg-slate-900/40 rounded-3xl border border-border-subtle hover:border-orange-500/20 transition-all">
          <Activity className="text-orange-400 mb-4" size={32} />
          <h4 className="font-bold text-text-primary text-lg mb-3">Smart Hydration</h4>
          <p className="text-sm text-text-muted leading-relaxed">Connected hydration systems that calculate fluid needs based on activity and climate.</p>
        </div>
      </div>
    </section>
        ...
        {[
          ...
          <div key={i} className="glass-card p-8 border-l-4 border-emerald-500/20 hover:bg-emerald-500/[0.02] transition-all">
            <div className="flex items-center gap-4 mb-4">
              ...
              <h4 className="text-xl font-bold text-text-primary">{hack.title}</h4>
            </div>
            <p className="text-sm text-text-muted leading-relaxed">{hack.desc}</p>
          </div>
        ))}
      </div>
    </section>
